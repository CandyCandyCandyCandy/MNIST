...
def conv_fc(filename):
    # 获取数据
    result = imageprepare(filename)
    ...
    with tf.Session() as sess:
        sess.run(init_op)
        print(result)
        saver.restore(sess, UPLOAD_PATH+"/package/model/fc_model.ckpt")
        prediction = tf.argmax(y_predict, 1)
        predint = prediction.eval(feed_dict={x: [result], keep_prob: 1.0}, session=sess)
        print(predint)
    return   "recognize result: %d" % predint[0] ,predint[0]
